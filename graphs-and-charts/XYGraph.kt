package com.klintsoft.graphsandcharts
...
fun XYGraph(
    ...
    Column(
        ...
        var tappedPointIndex: Int? by remember { mutableStateOf(null) }
        ...
        )
        {
            ...
            val scaleY = scaleYAxis.map { it.toString() }
                .ifEmpty { autoScale( axisValues =  points.map { it.y }, dimensionPx = graphHeight.toInt(), maxTextWidth = maxXScaleTextWidth, fontWidth = fontWidth) }
            ...
            val scaleX = scaleXAxis.map { it.toString() }
                .ifEmpty { //if scale for X axis is empty then autoScale
                    autoScale(points.map { it.x }, graphSize.width.toInt(), maxXScaleTextWidth, fontWidth = fontWidth)
                }
            ...
            if (labelY.isNotEmpty()) {
                ...
                rotate(-90f, pivot = rect.center) {
                    drawText(
                        ...
                        color = scaleColor,
                        topLeft = Offset(
                            newX.toFloat(), // 2f //- measuredYLabel.size.width / 2f,
                            newY.toFloat()
                        )
                    )
                }
